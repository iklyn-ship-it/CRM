...
function isAdminUser() {
  return ADMIN_EMAILS.has(normalizeEmail(authUserEmail));
}
function currentStorageScope() {
  return authUserId || normalizeEmail(authUserEmail) || "";
}
...
function serializeState() {
  return {
    ...
    calendarDate:
      state.calendarDate instanceof Date
        ? state.calendarDate.toISOString()
...
function applyState(raw) {
  ...
  state.equipment = raw.equipment || [];
  state.operators = raw.operators || [];
  state.orders = raw.orders || [];
  state.operations = raw.operations || [];
  state.repairs = raw.repairs || [];
  state.calendarDate = raw.calendarDate
    ? new Date(raw.calendarDate)
    : new Date();
  if (isNaN(state.calendarDate)) state.calendarDate = new Date();
  state.chartMode = raw.chartMode || "bars";
  state.calendarMode = raw.calendarMode || "month";
  state.integrations = {
    googleFormsUrl: raw.integrations?.googleFormsUrl || "",
    autoSync: Boolean(raw.integrations?.autoSync),
    importedResponseIds: Array.isArray(raw.integrations?.importedResponseIds)
      ? raw.integrations.importedResponseIds
      : [],
...
function loadLocalBackups(scope) {
  ...
  try {
    ...
    return Array.isArray(parsed)
      ? parsed.filter((item) => hasMeaningfulCloudState(item?.data))
      : [];
  } catch (_err) {
    return [];
  }
...
function saveLocalBackup(reason) {
  ...
  writeLocalBackups(
    [nextBackup, ...backups].slice(0, MAX_LOCAL_BACKUPS),
    scope,
  );
  return nextBackup;
}
...
function renderBackupStatus(kind, text) {
  ...
  if (!box) return;
  box.innerHTML = text
    ? `<div class="${kind === "error" ? "alert" : "ok"}">${esc(text)}</div>`
    : "";
}
function refreshBackupPanel() {
  ...
  select.innerHTML = backups.length
    ? backups
        .map(
          (item) =>
            `<option value="${item.id}">${new Date(item.createdAt).toLocaleString("uk-UA")} • ${esc(item.reason || "backup")}</option>`,
        )
        .join("")
...
function queueCloudSave() {
  ...
  cloudSaveChain = cloudSaveChain.catch(() => {}).then(() => saveCloudState());
...
function restoreSelectedBackup() {
  ...
  if (!backup)
    return renderBackupStatus("error", "Не удалось найти выбранный backup.");
  if (
    !confirm(
      "Восстановить CRM из выбранного backup? Текущее состояние будет заменено.",
    )
  )
    return;
  ...
  renderBackupStatus(
    "ok",
    `Восстановлен backup от ${new Date(backup.createdAt).toLocaleString("uk-UA")}.`,
  );
}
function createManualBackup() {
  ...
  refreshBackupPanel();
  if (!backup)
    return renderBackupStatus(
      "error",
      "Сначала войди в аккаунт, чтобы создать backup.",
    );
  renderBackupStatus(
    "ok",
    `Создан локальный backup: ${new Date(backup.createdAt).toLocaleString("uk-UA")}.`,
  );
}
...
async function saveCloudState() {
  ...
  const { error } = await supabaseClient
    ...
    .upsert(
      { user_id: authUserId, data: serializeState() },
      { onConflict: "user_id" },
    );
  if (error) {
  ...
  renderAuthStatus(
    "ok",
    `Авторизован: ${authUserId}. Данные синхронизируются с облаком.`,
  );
}
...
function statusBadge(s) {
  return (
    {
      new: badge("new", "Новое"),
      confirmed: badge("confirmed", "Подтверждена"),
      active: badge("active", "В работе"),
      completed: badge("completed", "Завершена"),
      cancelled: badge("cancelled", "Отменена"),
    }[s] || badge("new", "Новое")
  );
}
function repairStatusBadge(s) {
  return (
    {
      planned: badge("repairplan", "Запланирован"),
      active: badge("repairstatus", "В ремонте"),
      completed: badge("completed", "Завершён"),
      cancelled: badge("cancelled", "Отменён"),
    }[s] || badge("repairplan", "Запланирован")
  );
}
function typeBadge(t) {
  return t === "income"
    ? badge("income", "Приход")
    : badge("expense", "Расход");
}
function eqBadge(s) {
  return (
    {
      free: badge("free", "Свободна"),
      busy: badge("busy", "В работе"),
      repair: badge("repairstatus", "Ремонт"),
    }[s] || badge("free", "Свободна")
  );
}
function statusClass(status) {
  return `status-${String(status || "new").replace(/[^a-z]/g, "")}`;
}
function getFilterValue(id) {
  return (
    document.querySelector(`#${id} .filter-chip.active`)?.dataset.value || ""
  );
}
...
function orderPlan(order) {
  return (
    daysInclusive(order.startDate, order.endDate) * Number(order.rate || 0)
  );
}
function orderOps(orderId) {
  return state.operations.filter((o) => o.orderId === orderId);
}
function orderIncome(orderId) {
  return orderOps(orderId)
    .filter((o) => o.type === "income")
    .reduce((s, o) => s + Number(o.amount || 0), 0);
}
function orderExpense(orderId) {
  return orderOps(orderId)
    .filter((o) => o.type === "expense")
    .reduce((s, o) => s + Number(o.amount || 0), 0);
}
function orderProfit(orderId) {
  return orderIncome(orderId) - orderExpense(orderId);
}
function orderRemaining(order) {
  return Math.max(0, orderPlan(order) - orderIncome(order.id));
}
function repairOps(repairId) {
  return state.operations.filter((o) => o.repairId === repairId);
}
function repairExpense(repairId) {
  return repairOps(repairId)
    .filter((o) => o.type === "expense")
    .reduce((s, o) => s + Number(o.amount || 0), 0);
}
function allIncome() {
...
function runtimeEqStatus(eq) {
  ...
  const activeRepair = state.repairs.some(
    (r) =>
      r.equipmentId === eq.id &&
      r.status !== "cancelled" &&
      r.status !== "completed" &&
...
function utilForEq(eqId, date) {
  ...
  periods.forEach((p) => {
    ...
    const from = new Date(Math.max(s, monthStart)),
      to = new Date(Math.min(e, monthEnd));
    if (from <= to) {
      for (
    ...
    state.orders
      .filter(
        (o) =>
          ...
          overlap(o.startDate, o.endDate, r.startDate, r.endDate),
      )
      .forEach((o) => {
...
function orderEquipmentHtml(order) {
  ...
  if (String(order.notes || "").includes("Google Form"))
    return '<span class="badge repairplan">Требует разбора</span>';
  return "—";
}
...
function refreshSelects() {
  ...
  const eqOptions =
    ...
    state.equipment
      .map(
        (e) =>
          `<option value="${e.id}" data-rate="${e.defaultRate || 0}">${esc(e.name)}</option>`,
      )
      .join("");
  $("orderEquipment").innerHTML = eqOptions;
  $("repairEquipment").innerHTML = eqOptions;
  $("orderOperator").innerHTML =
...
function equipmentAnalytics() {
  return state.equipment
    .map((eq) => {
      ...
      const repairRelated = state.repairs.filter(
...
function renderDashboardChart() {
  ...
  if (!data.length) {
    wrap.innerHTML =
      '<div class="empty">Добавь технику, заявки, ремонты и операции — графики появятся здесь.</div>';
    return;
  }
  ...
  if (state.chartMode === "bars") {
    ...
    wrap.innerHTML =
      ...
      data
        .map(
          (x, i) =>
            `<div class="donut-item"><div><span class="dot" style="background:${colors[i % colors.length]}"></span> ${esc(x.name)}</div><div>${money(x.profit)}</div></div>`,
        )
        .join("") +
      `</div>
    </div>`;
...
function renderDashboard() {
  ...
  $("mOrders").textContent = state.orders.filter((o) =>
    ["new", "confirmed", "active"].includes(o.status),
  ).length;
  $("mRepairs").textContent = state.repairs.filter((r) =>
    ["planned", "active"].includes(r.status),
  ).length;
  const avgUtil = state.equipment.length
    ? Math.round(
        state.equipment.reduce((s, e) => s + utilForEq(e.id, new Date()), 0) /
  ...
  if (conf.length)
    alerts.push(
      `<div class="alert">Есть ${conf.length} конфликт(ов) по пересечению заявок на одну технику.</div>`,
    );
  else alerts.push(`<div class="ok">Конфликтов по аренде сейчас нет.</div>`);
  if (repConf.length)
    alerts.push(
      `<div class="alert">Есть ${repConf.length} конфликт(ов) между арендой и ремонтом.</div>`,
    );
  else
    alerts.push(
      `<div class="ok">Конфликтов между ремонтом и арендой нет.</div>`,
    );
  $("alerts").innerHTML = alerts.join("");

  const eqStats = equipmentAnalytics();
  $("topList").innerHTML = eqStats.length
    ? eqStats
        .map(
          (x, i) =>
            `<div class="item"><div><strong>${i + 1}. ${esc(x.name)}</strong><div class="small">Факт прибыли с учётом ремонтов</div></div><div><strong>${money(x.profit)}</strong></div></div>`,
        )
        .join("")
  ...
  const events = [
    ...state.orders.map((o) => ({
      ...
      endDate: o.endDate,
      label: esc(byId(state.clients, o.clientId)?.name || "—"),
      equipment: esc(byId(state.equipment, o.equipmentId)?.name || "—"),
      status: statusBadge(o.status),
      value: money(orderProfit(o.id)),
    })),
    ...state.repairs.map((r) => ({
      kind: "Ремонт",
      ...
      endDate: r.endDate,
      label: esc(r.tasks || "—"),
      equipment: esc(byId(state.equipment, r.equipmentId)?.name || "—"),
      status: repairStatusBadge(r.status),
      value: money(-repairExpense(r.id)),
    })),
  ]
  ...
  $("upcomingBody").innerHTML = events.length
    ? events
        .map(
          (e) =>
            `<tr><td>${fmtDate(e.startDate)} — ${fmtDate(e.endDate)}</td><td>${e.kind}</td><td>${e.label}</td><td>${e.equipment}</td><td>${e.status}</td><td>${e.value}</td></tr>`,
        )
        .join("")
    : `<tr><td colspan="6"><div class="empty">Пока нет событий.</div></td></tr>`;

  const linkedIncome = state.operations
    .filter((o) => o.type === "income" && o.orderId)
    .reduce((s, o) => s + Number(o.amount || 0), 0);
  const repairSpend = state.operations
    .filter((o) => o.type === "expense" && o.repairId)
    .reduce((s, o) => s + Number(o.amount || 0), 0);
  const linkedExpense = state.operations
  ...
  $("dashFinanceList").innerHTML = [
    ...
    .map(
      (x) =>
        `<div class="item"><div>${x[0]}</div><div><strong>${x[1]}</strong></div></div>`,
    )
    .join("");
}
function renderOrders() {
  ...
  $("conflictBox").innerHTML = conf.length
  ...
  if (q) {
    list = list.filter((o) => {
      ...
      const eq = (
        byId(state.equipment, o.equipmentId)?.name || ""
      ).toLowerCase();
  ...
  $("ordersBody").innerHTML = list.length
    ? list
        .map((o) => {
          ...
          const eq = orderEquipmentHtml(o);
          const confMark = confSet.has(o.id)
            ? `<div class="small" style="color:#fca5a5">Конфликт по технике</div>`
            : "";
          return `<tr>
      <td>${o.id.slice(-5)}</td>
      ...
      <td><button class="btn ghost" onclick="editOrder('${o.id}')">Редактировать</button><button class="btn ghost" onclick="removeOrder('${o.id}')">Удалить</button></td>
        ...
        .join("")
    : `<tr><td colspan="10"><div class="empty">Ничего не найдено.</div></td></tr>`;
}
function renderRepairs() {
  ...
  const confSet = new Set(conf.flatMap((x) => [x[0], x[1]]));
  $("repairConflictBox").innerHTML = conf.length
  ...
  if (q) {
    list = list.filter((r) => {
      const eq = (
        byId(state.equipment, r.equipmentId)?.name || ""
      ).toLowerCase();
      return (
        eq.includes(q) ||
  ...
  list.sort((a, b) => b.startDate.localeCompare(a.startDate));
  $("repairsBody").innerHTML = list.length
    ? list
        .map((r) => {
          const eq = esc(byId(state.equipment, r.equipmentId)?.name || "—");
          const confMark = confSet.has(r.id)
            ? `<div class="small" style="color:#fca5a5">Пересекается с арендой</div>`
            : "";
          return `<tr>
      <td>${r.id.slice(-5)}</td>
      ...
      <td><button class="btn ghost" onclick="editRepair('${r.id}')">Редактировать</button><button class="btn ghost" onclick="removeRepair('${r.id}')">Удалить</button></td>
        ...
        .join("")
    : `<tr><td colspan="7"><div class="empty">Ремонтов пока нет.</div></td></tr>`;
}
function renderCalendarMonth() {
  ...
  const start = new Date(y, m, 1),
    ...
    startWeek = (start.getDay() + 6) % 7,
    dim = end.getDate();
  const names = ["Пн", "Вт", "Ср", "Чт", "Пт", "Сб", "Вс"];
  let html =
    `<div class="cal">` +
    names.map((n) => `<div class="cal-h">${n}</div>`).join("");
  let cell = 1 - startWeek,
    weeks = Math.ceil((startWeek + dim) / 7);
  for (let w = 0; w < weeks; w++) {
    for (let i = 0; i < 7; i++, cell++) {
      const cur = new Date(y, m, cell);
      const inMonth = cur.getMonth() === m;
      const ds = cur.toISOString().slice(0, 10);
      const rentEntries = state.orders
        .filter(
          (o) =>
            isCalendarVisibleOrder(o) && o.startDate <= ds && o.endDate >= ds,
        )
        .map((o) => {
          const eq = esc(
            byId(state.equipment, o.equipmentId)?.name || "Техника",
          );
          const cl = esc(byId(state.clients, o.clientId)?.name || "Клиент");
          return {
            ...o,
      ...
      const repairEntries = state.repairs
        .filter(
          (r) =>
        ...
        .map((r) => {
          const eq = esc(
            byId(state.equipment, r.equipmentId)?.name || "Техника",
          );
      ...
      html +=
        `<div class="cal-c" style="${inMonth ? "" : "opacity:.45"}"><div><strong>${cur.getDate()}</strong> <span class="small">${cur.toLocaleDateString("ru-RU", { month: "short" })}</span></div>` +
        entries
          .slice(0, 5)
          .map(
            (e) =>
              `<div class="event ${e.type === "repair" ? "repair" : e.statusClass} ${cnt[e.equipmentId] > 1 ? "conflict" : ""}">${e.eq}<br><span class="small">${e.type === "repair" ? "Ремонт: " : "Аренда: "}${e.cl}</span></div>`,
          )
          .join("") +
        (entries.length > 5
          ? `<div class="small" style="margin-top:6px">ещё ${entries.length - 5}</div>`
          : "") +
        `</div>`;
    }
  }
...
function renderCalendarTimeline() {
  ...
  const allConflicts = new Set([
    ...orderConflicts().map((x) => x[0]),
    ...orderConflicts().map((x) => x[1]),
    ...repairConflicts().map((x) => x[0]),
    ...repairConflicts().map((x) => x[1]),
  ]);
  ...
  state.equipment.forEach((eq) => {
    ...
    const rowEvents = [
      ...state.orders
        .filter((o) => o.equipmentId === eq.id && isCalendarVisibleOrder(o))
        .map((o) => ({
          id: o.id,
          type: "rent",
          status: o.status,
          title: esc(byId(state.clients, o.clientId)?.name || "Аренда"),
          startDate: o.startDate,
          endDate: o.endDate,
          conflict: allConflicts.has(o.id),
        })),
      ...state.repairs
        ...
        .map((r) => ({
          id: r.id,
          type: "repair",
          title: esc(r.tasks || "Ремонт"),
          startDate: r.startDate,
          endDate: r.endDate,
          conflict: allConflicts.has(r.id),
        })),
    ];
...
function renderCalendar() {
  const d = state.calendarDate;
  $("monthLabel").textContent = d.toLocaleDateString("ru-RU", {
    month: "long",
    year: "numeric",
  });
...
function renderFinance() {
  const q = $("opSearch").value.trim().toLowerCase(),
    ft = getFilterValue("opFilterType");
  ...
  $("operationsBody").innerHTML = ops.length
    ? ops
        .map((o) => {
          let linkText = "—";
          if (o.orderId) {
            ...
            if (ord)
              linkText = `Аренда ${ord.id.slice(-5)} • ${byId(state.clients, ord.clientId)?.name || ""}`;
          } else if (o.repairId) {
            const rep = byId(state.repairs, o.repairId);
            if (rep)
              linkText = `Ремонт ${rep.id.slice(-5)} • ${byId(state.equipment, rep.equipmentId)?.name || ""}`;
          }
          return `<tr>
      ...
      <td><button class="btn ghost" onclick="editOperation('${o.id}')">Редактировать</button><button class="btn ghost" onclick="removeOperation('${o.id}')">Удалить</button></td>
        ...
        .join("")
    : `<tr><td colspan="7"><div class="empty">Финансовых операций пока нет.</div></td></tr>`;

  $("financeByOrderBody").innerHTML = state.orders.length
    ? state.orders
        .map((o) => {
          const cl = esc(byId(state.clients, o.clientId)?.name || "—");
        ...
        .join("")
    : `<tr><td colspan="7"><div class="empty">Нет заявок.</div></td></tr>`;

  $("financeByRepairBody").innerHTML = state.repairs.length
    ? state.repairs
        .map((r) => {
          const eq = esc(byId(state.equipment, r.equipmentId)?.name || "—");
        ...
        .join("")
    : `<tr><td colspan="5"><div class="empty">Нет ремонтов.</div></td></tr>`;
}
function renderEquipment() {
  $("equipmentBody").innerHTML = state.equipment.length
    ? state.equipment
        .map((eq) => {
          const repairs = state.repairs.filter(
            (r) => r.equipmentId === eq.id && r.status !== "cancelled",
          ).length;
        ...
        .join("")
    : `<tr><td colspan="8"><div class="empty">Нет техники.</div></td></tr>`;
}
function renderClients() {
  $("clientsBody").innerHTML = state.clients.length
    ? state.clients
        .map((c) => {
          const ords = state.orders.filter((o) => o.clientId === c.id);
          const income = ords.reduce((s, o) => s + orderIncome(o.id), 0);
          const prof = ords.reduce((s, o) => s + orderProfit(o.id), 0);
        ...
        .join("")
    : `<tr><td colspan="7"><div class="empty">Нет клиентов.</div></td></tr>`;
}
function renderOperators() {
  $("operatorsBody").innerHTML = state.operators.length
    ? state.operators
        .map((op) => {
          const ords = state.orders.filter(
            (o) => o.operatorId === op.id && o.status !== "cancelled",
          );
          const shifts = ords.reduce(
            (s, o) => s + daysInclusive(o.startDate, o.endDate),
            0,
          );
        ...
        .join("")
    : `<tr><td colspan="6"><div class="empty">Нет операторов.</div></td></tr>`;
}
...
function setAppAccess(isAuthorized) {
  document.querySelectorAll(".section").forEach((section) => {
    ...
    section.style.display = isAuthorized
      ? isAuthSection
        ? "none"
        : ""
      : isAuthSection
  ...
  if ($("topActions"))
    $("topActions").style.display = isAuthorized ? "" : "none";
  ...
  if (!isAuthorized) {
    ...
    if ($("pageSubtitle"))
      $("pageSubtitle").textContent =
        "Войди в аккаунт для доступа к данным CRM.";
  } else if (
    $("section-auth") &&
    $("section-auth").classList.contains("active")
  ) {
...
function updateAuthUi(session) {
  ...
  if (user) {
    ...
    renderAuthStatus(
      "error",
      "Не авторизован. Войди, чтобы открыть CRM и синхронизацию.",
    );
  }
  refreshAdminControls();
  refreshBackupPanel();
...
function refreshIntegrationForm() {
  ...
  const details = state.integrations.lastSyncAt
    ? `${state.integrations.lastSyncStatus} Последняя синхронизация: ${new Date(state.integrations.lastSyncAt).toLocaleString("uk-UA")}.`
    : state.integrations.lastSyncStatus;
  if (details) {
    renderIntegrationStatus(
      state.integrations.lastSyncStatus.startsWith("Ошибка") ? "error" : "ok",
      details,
    );
  } else {
...
function findEquipmentId(record) {
  ...
  const eq = state.equipment.find(
    (item) =>
      (code && normalizeText(item.code) === code) ||
...
function findOperatorId(record) {
  ...
  return name
    ? state.operators.find((item) => normalizeText(item.name) === name)?.id ||
        ""
    : "";
}
function ensureClient(record) {
  ...
  let client = state.clients.find(
    (item) =>
      (phone && normalizeText(item.phone) === phone) ||
      (name && normalizeText(item.name) === normalizeText(name)),
  );
  ...
  client = {
    ...
    source: String(
      record.clientSource || record.sourceLabel || "Google Form",
    ).trim(),
    type: "Разовый",
...
function buildImportedOrder(record) {
  ...
  const notes = [
    ...
    !equipmentId && (record.equipmentCode || record.equipmentName)
      ...
      : "",
    record.responseId
      ? `Google Form ID: ${record.responseId}`
      : "Google Form import",
  ]
    .filter(Boolean)
    .join("\n");
...
function importGoogleFormResponses(records) {
  ...
  records.forEach((record) => {
    ...
    if (
      (responseId &&
        state.integrations.importedResponseIds.includes(responseId)) ||
      !record.clientName ||
      !startDate ||
      !endDate
    ) {
  ...
  state.integrations.lastSyncStatus = imported
    ? `Импортировано заявок: ${imported}. Пропущено: ${skipped}.`
    : `Новых заявок не найдено. Пропущено: ${skipped}.`;
  renderAll();
  refreshIntegrationForm();
}
...
function syncGoogleForms(manual) {
  ...
  if (!url) {
    renderIntegrationStatus(
      "error",
      "Сначала укажи URL Google Apps Script Web App.",
    );
    return;
  }
  ...
  script.onerror = () => {
    ...
    state.integrations.lastSyncAt = new Date().toISOString();
    state.integrations.lastSyncStatus =
      "Ошибка синхронизации: не удалось загрузить данные из Google Apps Script.";
    refreshIntegrationForm();
  };
...
function clearEquipmentForm() {
  ...
  $("equipmentForm").reset();
  $("eqStatus").value = "free";
  $("equipmentFormTitle").textContent = "Добавить технику";
  $("equipmentSaveBtn").textContent = "Сохранить технику";
  $("equipmentCancelEditBtn").classList.add("hidden");
}
function editEquipment(id) {
  const eq = byId(state.equipment, id);
  if (!eq) return;
  $("equipmentId").value = eq.id;
  $("eqName").value = eq.name || "";
  $("eqType").value = eq.type || "";
  $("eqCode").value = eq.code || "";
  $("eqRate").value = eq.defaultRate || 0;
  $("eqStatus").value = eq.status || "free";
  $("equipmentFormTitle").textContent = "Редактирование техники";
  $("equipmentSaveBtn").textContent = "Сохранить изменения";
  $("equipmentCancelEditBtn").classList.remove("hidden");
  activateSection("equipment");
}
function clearClientForm() {
  $("clientId").value = "";
  $("clientForm").reset();
  $("clientType").value = "Разовый";
  $("clientFormTitle").textContent = "Добавить клиента";
  $("clientSaveBtn").textContent = "Сохранить клиента";
...
function editClient(id) {
  ...
  $("clientSource").value = client.source || "";
  $("clientType").value = client.type || "Разовый";
  $("clientNotes").value = client.notes || "";
  $("clientFormTitle").textContent = "Редактирование клиента";
  $("clientSaveBtn").textContent = "Сохранить изменения";
...
function clearOperatorForm() {
  $("operatorId").value = "";
  $("operatorForm").reset();
  $("operatorRate").value = 0;
  $("operatorFormTitle").textContent = "Добавить оператора";
  $("operatorSaveBtn").textContent = "Сохранить оператора";
  $("operatorCancelEditBtn").classList.add("hidden");
}
function editOperator(id) {
  const op = byId(state.operators, id);
  if (!op) return;
  $("operatorId").value = op.id;
  $("operatorName").value = op.name || "";
  $("operatorPhone").value = op.phone || "";
  $("operatorSkill").value = op.skill || "";
  $("operatorRate").value = op.rate || 0;
  $("operatorFormTitle").textContent = "Редактирование оператора";
  $("operatorSaveBtn").textContent = "Сохранить изменения";
  $("operatorCancelEditBtn").classList.remove("hidden");
  activateSection("operators");
}
function clearOrderForm() {
  $("orderId").value = "";
  $("orderForm").reset();
  setOrderStatusPicker("new");
  $("orderFormTitle").textContent = "Новая заявка";
  $("orderSaveBtn").textContent = "Сохранить заявку";
  $("orderCancelEditBtn").classList.add("hidden");
}
function editOrder(id) {
  ...
  $("orderEquipment").value = o.equipmentId || "";
  $("orderOperator").value = o.operatorId || "";
  $("orderStart").value = o.startDate;
  $("orderEnd").value = o.endDate;
  $("orderLocation").value = o.location || "";
  $("orderRate").value = o.rate || 0;
  setOrderStatusPicker(o.status || "new");
  $("orderNotes").value = o.notes || "";
  $("orderFormTitle").textContent = "Редактирование заявки";
  $("orderSaveBtn").textContent = "Сохранить изменения";
  $("orderCancelEditBtn").classList.remove("hidden");
  activateSection("orders");
}
function clearRepairForm() {
  $("repairId").value = "";
  $("repairForm").reset();
  $("repairStatus").value = "planned";
  $("repairFormTitle").textContent = "Запланировать ремонт";
  $("repairSaveBtn").textContent = "Сохранить ремонт";
  $("repairCancelEditBtn").classList.add("hidden");
}
function editRepair(id) {
  const r = byId(state.repairs, id);
  if (!r) return;
  $("repairId").value = r.id;
  $("repairEquipment").value = r.equipmentId || "";
  $("repairStart").value = r.startDate;
  $("repairEnd").value = r.endDate;
  $("repairStatus").value = r.status || "planned";
  $("repairTasks").value = r.tasks || "";
  $("repairNotes").value = r.notes || "";
  $("repairFormTitle").textContent = "Редактирование ремонта";
  $("repairSaveBtn").textContent = "Сохранить изменения";
  $("repairCancelEditBtn").classList.remove("hidden");
...
function clearOperationForm() {
  ...
  $("operationForm").reset();
  $("opType").value = "income";
  $("opCategory").value = "Оплата клиента";
  $("opOrder").value = "";
  $("opRepair").value = "";
  $("opFormTitle").textContent = "Новая финансовая операция";
  $("opSaveBtn").textContent = "Сохранить операцию";
  $("opCancelEditBtn").classList.add("hidden");
}
function editOperation(id) {
  const o = byId(state.operations, id);
  if (!o) return;
  $("operationId").value = o.id;
  $("opDate").value = o.date;
  $("opType").value = o.type;
  $("opCategory").value = o.category || "Прочее";
  $("opAmount").value = o.amount;
  $("opOrder").value = o.orderId || "";
  $("opRepair").value = o.repairId || "";
  $("opComment").value = o.comment || "";
  $("opFormTitle").textContent = "Редактирование операции";
  $("opSaveBtn").textContent = "Сохранить изменения";
  $("opCancelEditBtn").classList.remove("hidden");
...
function removeOrder(id) {
  if (
    !confirm(
      "Удалить заявку? Все привязанные к ней операции останутся, но связь будет снята.",
    )
  )
    return;
  state.operations = state.operations.map((op) =>
    op.orderId === id ? { ...op, orderId: "" } : op,
  );
...
function removeRepair(id) {
  if (
    !confirm(
      "Удалить ремонт? Все привязанные к нему операции останутся, но связь будет снята.",
    )
  )
    return;
  state.operations = state.operations.map((op) =>
    op.repairId === id ? { ...op, repairId: "" } : op,
  );
...
function removeClient(id) {
  if (confirm("Удалить клиента и связанные заявки?")) {
    const orderIds = state.orders
      .filter((o) => o.clientId === id)
      .map((o) => o.id);
    state.operations = state.operations.map((op) =>
      orderIds.includes(op.orderId) ? { ...op, orderId: "" } : op,
    );
...
function removeOperator(id) {
  if (confirm("Удалить оператора?")) {
    state.orders = state.orders.map((o) =>
      o.operatorId === id ? { ...o, operatorId: "" } : o,
    );
...
function bindForms() {
  $("clientForm").addEventListener("submit", (e) => {
    ...
    const payload = {
      ...
      source: $("clientSource").value.trim(),
  ...
  $("equipmentForm").addEventListener("submit", (e) => {
    ...
    const payload = {
      ...
      defaultRate: Number($("eqRate").value || 0),
      status: $("eqStatus").value,
    };
    if (id) {
      const idx = state.equipment.findIndex((x) => x.id === id);
      if (idx > -1)
        state.equipment[idx] = { ...state.equipment[idx], ...payload };
  ...
  $("operatorForm").addEventListener("submit", (e) => {
    ...
    const payload = {
      ...
      skill: $("operatorSkill").value.trim(),
      rate: Number($("operatorRate").value || 0),
    };
    if (id) {
      const idx = state.operators.findIndex((x) => x.id === id);
      if (idx > -1)
        state.operators[idx] = { ...state.operators[idx], ...payload };
    } else state.operators.push({ id: uid("op"), ...payload });
    clearOperatorForm();
    renderAll();
  });
  $("orderEquipment").addEventListener("change", (e) => {
    const eq = byId(state.equipment, e.target.value);
    if (eq && !$("orderRate").value) $("orderRate").value = eq.defaultRate || 0;
  });
  $("orderForm").addEventListener("submit", (e) => {
    e.preventDefault();
    const id = $("orderId").value.trim(),
      clientId = $("orderClient").value,
      equipmentId = $("orderEquipment").value,
      startDate = $("orderStart").value,
      endDate = $("orderEnd").value;
    if (!clientId || !equipmentId || !startDate || !endDate)
      return alert("Заполни клиента, технику и даты.");
    if (startDate > endDate)
      return alert("Дата окончания не может быть раньше даты начала.");
    const payload = {
      clientId,
      equipmentId,
      operatorId: $("orderOperator").value,
      startDate,
      endDate,
      location: $("orderLocation").value.trim(),
      rate: Number($("orderRate").value || 0),
  ...
  $("repairForm").addEventListener("submit", (e) => {
    ...
    const id = $("repairId").value.trim(),
      equipmentId = $("repairEquipment").value,
      startDate = $("repairStart").value,
      endDate = $("repairEnd").value;
    if (!equipmentId || !startDate || !endDate)
      return alert("Заполни технику и даты ремонта.");
    if (startDate > endDate)
      return alert("Дата окончания не может быть раньше даты начала.");
    const payload = {
      equipmentId,
      ...
      tasks: $("repairTasks").value.trim(),
      notes: $("repairNotes").value.trim(),
    };
    if (id) {
      const idx = state.repairs.findIndex((x) => x.id === id);
      if (idx > -1) state.repairs[idx] = { ...state.repairs[idx], ...payload };
  ...
  $("operationForm").addEventListener("submit", (e) => {
    e.preventDefault();
    const id = $("operationId").value.trim(),
      date = $("opDate").value,
      type = $("opType").value,
      category = $("opCategory").value,
      amount = Number($("opAmount").value || 0),
      orderId = $("opOrder").value,
      repairId = $("opRepair").value,
    ...
    if (id) {
      ...
      if (idx > -1)
        state.operations[idx] = { ...state.operations[idx], ...payload };
  ...
  $("opOrder").addEventListener("change", () => {
    if ($("opOrder").value) $("opRepair").value = "";
  });
  $("opRepair").addEventListener("change", () => {
    if ($("opRepair").value) $("opOrder").value = "";
  });
}
function activateSection(sec) {
  ...
  document
    ...
    .forEach((x) => x.classList.toggle("active", x.dataset.sec === sec));
  const map = {
    auth: ["Авторизация", "Только вход в систему и управление доступом"],
    dashboard: [
      "Дашборд",
      "Контроль выручки, расходов, прибыли, ремонтов и загрузки",
    ],
    ...
    integrations: [
      "Google Forms",
      "Подключение и синхронизация заявок из Google Forms",
    ],
...
function setOrderStatusPicker(value) {
  ...
  wrap
    .querySelectorAll(".filter-chip")
    .forEach((item) =>
      item.classList.toggle("active", item.dataset.value === nextValue),
    );
}
...
function bindGeneral() {
  ...
  on("currentMonth", "click", () => {
    state.calendarDate = new Date();
    renderCalendar();
    save();
  });
  on("calendarMode", "change", () => {
  ...
  on("resetBtn", "click", () => {
    if (!isAdminUser())
      return alert("Сброс доступен только для iklyn@rbt-group.com.ua.");
  ...
  on("authSignupBtn", "click", async () => {
    ...
    renderAuthStatus(
      "ok",
      "Регистрация успешна. Проверь email, если включено подтверждение.",
    );
  });
...
function seedDemo() {
  if (!isAdminUser())
    return alert("Демо-данные доступны только для iklyn@rbt-group.com.ua.");
  if (
...
async function init() {
  ...
  refreshBackupPanel();
  await initAuth();
  if (state.integrations.autoSync && state.integrations.googleFormsUrl)
